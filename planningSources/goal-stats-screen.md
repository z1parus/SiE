...
1. **Полноэкранный экран** — `Navigator.push` с полноэкранным `Scaffold`.

2. **Прогресс-кольцо** — `CustomPaint`. Подходит под тёмный стиль приложения, не требует лишней зависимости. Реализация: `Arc` от `-π/2` на угол `2π * progress/100`, толщина 10–12 dp, цвет `goal.color`, фоновая окружность `sc.border`.

3. **Имена привычек** — кросс-чтение через `ref.watch(habitsProvider)`. В карточке «Привязанные привычки» выводить список названий: для каждого `GoalHabitLink` найти `Habit` по `habitId` в `HabitsState.habits`. Если привычка не найдена (удалена) — показывать `«Привычка удалена»` серым цветом.

4. **`_StatusChip`, `_categoryIcon`, `_categoryColor`, `_categoryLabel`** — дублируем в `goal_stats_screen.dart`.
