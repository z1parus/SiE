...
1. **Дублирование запросов**: при наличии уникального ограничения `(requester_id, addressee_id)` пользователь A не может отправить запрос пользователю B, если тот уже отправил запрос пользователю A. Нужно ли проверять «зеркальный» запрос? Вероятно, да — при `sendRequest` проверять, нет ли входящего запроса от этого пользователя, и если есть — автопринимать.

2. **Declined-статус**: стоит ли хранить отклонённые запросы в базе? Если да — можно блокировать повторные запросы на некоторое время. Если нет — удалять запись при отклонении (`DELETE` вместо `UPDATE`). Простой вариант: удалять.

3. **Загрузка `PublicProfile` для notifications**: для отображения аватара/имени в листе уведомлений нужен JOIN с `profiles`. Либо хранить `username` в поле `payload JSONB` при создании уведомления триггером, либо делать JOIN в провайдере. Второй вариант чище, но требует доступа к таблице `profiles` (RLS уже есть — `profiles` публично читаемы).

4. **Realtime на Android**: у некоторых пользователей Supabase Realtime нестабилен при переходе приложения в фон. Стоит ли добавить `refresh on resume` (переподключение при возврате в foreground)? Потенциально — да, через `WidgetsBindingObserver`.

5. **Лимит уведомлений**: запрашивать последние N (например, 50) уведомлений с пагинацией или без? Для первой итерации достаточно загружать последние 50 без пагинации.

6. **Удаление vs. архивирование уведомлений**: нажатие «Прочитать все» только помечает как прочитанные или физически удаляет? Рекомендация: только `is_read = true`, чтобы история сохранялась.

7. **Сортировка друзей в списке**: алфавитно по `username` или по дате добавления? Для первой итерации — по дате добавления (`createdAt DESC`), самые новые сверху.
