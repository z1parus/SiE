...
1. **Конфиг дыхания в пресете:** оба варианта на выбор — «быстрые параметры» (раунды/циклы/задержки, `breathingConfigJson`) ИЛИ ссылка на сохранённую последовательность (`breathingSequenceId`).
2. **Переход:** короткий авто-переходный экран (2–3 сек) «Дыхание завершено — переходим к медитации», далее автостарт медитации без тапа.
3. **XP/DP:** единый за всю цепочку — дыхательная сессия пишется с метриками и `xpAwarded=0`, XP/DP начисляет медитация один раз по суммарной длительности.
4. **Опрос состояния:** один — `stateBefore` перед дыханием, `stateAfter` после медитации; отдельную рефлексию дыхания (calmness/mood) в цепочке пропускаем.
5. **Схема — аддитивная:** legacy-колонки `breathingPatternId`/`breathingDurationMin` остаются (не используются), добавляются новые поля и линки.
