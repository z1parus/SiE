...
> **Статус: реализовано (2026-06-14).** В оба экрана добавлен 4-й таб «УЗОРЫ».
> Магазин: покупка/оснащение узоров переиспользуют существующие `_onBuy`/
> `_onEquip`, превью узора (`ProfilePatternThumb` в сетке, анимированный
> `ProfilePatternLayer` в полноэкранном preview). Облик: выбор узора с live-
> preview композита (фон + узор через `ProfilePatternLayer`), снятие узора —
> повторным тапом по выбранному. Все `switch (asset.type)` дополнены кейсом
> `profilePattern`. Отдельная плитка «Без узора» и пользовательский выбор
> оттенка не вводились (снятие — тапом; цвет наследует акцент фона).
