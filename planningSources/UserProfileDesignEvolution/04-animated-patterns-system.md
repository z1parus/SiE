...
- Сид-данные: 4 стартовых узора с ценами (предложение):
  `neural_threads` — common, 0 DP (стартовый);
  `dot_matrix` — common, 300 DP;
  `low_poly` — rare, 800 DP;
  `iso_grid` — epic, 1500 DP.

## Открытые вопросы

1. Шейдер vs `CustomPainter` по каждому узору — финализировать после бенчмарка
   (особенно `low_poly`/`iso_grid` на слабых Android).
2. Цены и редкость стартовых узоров — подтвердить экономику.
3. Накладывать ли узор и на дефолтный (не кастомный) фон, или только при
   наличии кастомного фона. Рекомендую разрешать на любом фоне — узор самоценен.
4. Нужен ли узору собственный цвет (`pattern_color`) независимо от
   `accentColor` фона, или всегда наследовать акцент фона.
5. Анимация превью в сетке: ленивая по видимости vs статичный кадр — выбрать по
   бюджету производительности списка.
