...
class SessionCompletionNotifier extends Notifier<void> {
  ...
  /// Records a breathing session that ran as the **preliminary practice of a
  /// meditation** (Ecosystem Stage 1). Metrics are stored and linked to the
  /// meditation via [meditationSessionId], but no XP/DP/achievement is granted
  /// here — the meditation awards a single unified reward for the whole chain.
  /// Both ids are pre-generated by the caller so the two rows cross-link.
  Future<void> recordChainedBreathing({
    required String breathingSessionId,
    required String meditationSessionId,
    required int durationSeconds,
    int breaths = 0,
    int rounds = 0,
    int longestHoldSeconds = 0,
    ...
    await db.insertBreathingSession(LocalBreathingSessionsCompanion(
      ...
      longestHoldSeconds: Value(longestHoldSeconds),
      totalHoldSeconds: Value(totalHoldSeconds),
      meditationSessionId: Value(meditationSessionId),
      synced: Value(isOnline),
    ));
    ...
    if (isOnline) {
      try {
        await client.from('breathing_sessions').insert(sessionRow);
        await db.markBreathingSessionSynced(breathingSessionId);
      } catch (e) {
        debugPrint('SiE ChainedBreathing: insert failed, queuing — $e');
        await db.enqueueSyncOp(
            'insert_breathing_session', jsonEncode(sessionRow));
      }
    } else {
      await db.enqueueSyncOp('insert_breathing_session', jsonEncode(sessionRow));
    }
